...
def open_image():
    ...
    if file_path:
        ...
        input_image = input_image.resize((32,32), Image.ANTIALIAS if hasattr(Image, "ANTIALIAS") else Image.BICUBIC)
        input_image = ImageTk.PhotoImage(input_image)
        canvas.create_image(0, 0, anchor=tk.NW, image=input_image)
        print("Image loaded successfully.")
        grayscale_image = input_image.convert('L')
        input_array = np.array(grayscale_image)
        flattened_input = input_array.flatten().reshape(1, -1)
...
def run_genetic_algorithm():
    ...
    if input_image is None:
        ...
        if save_path:
            chromosomes, output = genetic_algorithm(target_vector, 100, 10, 100, 0.1, 0.8, flattened_input, hidden_layer1_weight_sets, hidden_output_weight_sets, save_path)
            ...
            for i, chromosome in enumerate(chromosomes, start=1):
                ...
                output_output.insert(tk.END, f"Iteration {i} - Output with GA: {output[i]}\n")
